...
export class ModxProxyService {
    ...
    async login(username, password, baseUrl) {
        try {
            ...
            const response = await this.httpClient.post(connectorUrl, loginData, {
                headers: {
     ...
     */
    async getProcessors(refresh = false) {
        ...
        try {
            ...
            const response = await this.httpClient.post(connectorUrl, processorData, {
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
                    'X-Requested-With': 'XMLHttpRequest',
                    'Referer': refererUrl,
     ...
     */
    async callProcessor(namespace, action, data = {}) {
        if (!this.isAuthenticated) {
            throw new Error('Not authenticated. Please login first.');
        }
        try {
...
//# sourceMappingURL=modx-proxy.js.map